...
import {
  ...
  getTicket,
  getSession,
} from "../middleware/redux/selectors";
...
export default class TicketsScreen extends Component {
  ...
  renderItem = ({ item }) => {
    ...
    const header =
      item.number +
      (item.status ? ", " + item.status.name.toString().toLowerCase() : "");
    var name = item.visitorFullName ? item.visitorFullName : " ";

    if (item.carNumber) name = item.carNumber;

    if (item.type.id == "3724900074000") name = item.whatHappened;

    try {
      try {
        var type =
          item.type && item.type.name + " " + item.visitDate.substring(0, 10);
        if (item.expirationDate) {
          type += " - " + item.expirationDate.substring(0, 10);
        }
      } catch {
          ...
          >
            ...
            >
              <View
                style={{
                  width: 10,
                  backgroundColor: status2colors[item.status && item.status.id],
                  borderBottomLeftRadius: 10,
                  borderTopLeftRadius: 10,
                }}
              ></View>
              <View
                style={{ flexDirection: "column", marginLeft: 5, width: "90%" }}
              >
