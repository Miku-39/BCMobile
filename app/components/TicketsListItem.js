  ...
  render() {
    ...
    const showAlert = () => {
      if (
        ...
        item.type.shortName == "Внос" ||
        item.type.shortName == "Вынос"
      ) {
        Alert.alert(
          "Информация",
          `${
            item.carModelText
              ? "Машина: " + item.carModelText + " " + item.carNumber + "\n\n"
              : ""
          }` +
            `${
            ...
            `${
              item.materialValuesData
                ? "Груз: " + item.materialValuesData + "\n\n"
                : ""
            }` +
            `${
              item.materialValuesSize == null
                ? ""
                : "Габариты груза: " + item.materialValuesSize + "\n\n"
            }` +
            `${
              item.longterm
                ? "Срок действия: " + item.expirationDate + "\n\n"
                : ""
            }` +
            `${
              item.rejectionReason
    ...
    const header = () => {
      if (item.visitDate) {
        if (item.parking) {
          if (
            item.parking.id == "3714666676000" ||
            item.parking.name == "Постоянные ТС"
          ) {
            arrivalDate = " ";
          } else {
            arrivalDate = item.visitDate.substr(0, 10);
          }
        } else {
          arrivalDate = item.visitDate.substr(0, 10);
        }
      ...
      return (
        <View style={{ flexDirection: "row" }}>
          <Text
            style={{
              fontSize: 11,
              color: status2colors[item.status && item.status.id],
              fontStyle: "italic",
              marginRight: 5,
            }}
          >
          ...
          <Text style={{ fontSize: 11, color: "#767878", fontStyle: "italic" }}>
            {`№ ${item.number} ${
              item.visitDate ? "от " + item.visitDate.split("T")[0] : ""
            } ${
              item.longterm ? "до " + item.expirationDate.split("T")[0] : ""
            }`}
          </Text>
        </View>
      );
              ...
              >
                <View
                  style={{
                    width: 10,
                    backgroundColor:
                      status2colors[item.status && item.status.id],
                    borderRadius: 5,
                    margin: 1,
                  }}
                ></View>
                <View style={{ flexDirection: "column", marginLeft: 8 }}>
                  ...
                  <View style={{ flexDirection: "row" }}>
                    <Text style={{ fontSize: 16, color: "#767878" }}>
                      {item.parking ? item.parking.name : "Парковка не указана"}
                    </Text>
                    <Text
                      style={{
                        fontSize: 16,
                    ...
                    >
                      {" " + ((item.parkingPlace && item.parkingPlace) || "")}
                    </Text>
                  </View>
            ...
            <TouchableHighlight onPress={showAlert} underlayColor="#FFFFFF">
              ...
              >
                ...
                >
                  ...
                  <View style={{ flexDirection: "row" }}>
                    <Text style={{ fontSize: 18, color: "black" }}>
                      {(item.visitorFullName && item.visitorFullName) ||
                        "ФИО не указано"}
                    </Text>
                  </View>
