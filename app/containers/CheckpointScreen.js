...
export default class CheckpointScreen extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (fetched) this.list.reloadData();
  ...
  _handleShowSearchBarClick = () => {
    ...
    if (!searchBarIsShown) this.setState({ searchBarIsShown: true });
  };
  ...
  handleSearch = (text) => {
    ...
    if (text) {
      data = items.filter(
        (item) =>
          (item.carNumber &&
            item.carNumber.toLowerCase().includes(text.toLowerCase())) ||
          (item.carModelText &&
            item.carModelText.toLowerCase().includes(text.toLowerCase())) ||
          (item.visitorFullName &&
            item.visitorFullName.toLowerCase().includes(text.toLowerCase())) ||
          (item.number && item.number + "")
            .toLowerCase()
            .includes(text.toLowerCase()) ||
          (item.type.shortName &&
            item.type.shortName.toLowerCase().includes(text.toLowerCase()))
      );
    } else {
      data = items;
    }
